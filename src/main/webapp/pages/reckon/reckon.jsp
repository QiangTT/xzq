...
<!--菜单标题栏开始-->
	<table width='100%' height='31' border='0' cellspacing='0' cellpadding='0' background='${pageContext.request.contextPath}/pages/images/index_26.jpg'>
		<tr>
		<td width='18%'>&nbsp;</td>
		<td width='47%' valign='middle' background='${pageContext.request.contextPath}/pages/images/ctitle_bak_fcenter.jpg'><span class="channel_title">帐单管理<!-- 模块名称 -->></span></td>
	...
	<tr>
		...
		<table border='0' cellspacing='0' cellpadding='0'>
			<tr>
				...
				<a href='index.jsp' class=channel_title>帐单管理首页</a>
				<img src="${pageContext.request.contextPath}/pages/images/sy_2_18.jpg" border="0">
				<a href='${pageContext.request.contextPath}/pages/reckon/reckon.jsp' class=channel_title>帐单查询</a>
				</td>
			</tr>
		</table>
		</td>
...
<table width='100%' border='1' cellspacing='0' cellpadding='0'>
	<tr width="100%" height="10%">
		...
		<form name="reckonform" method="post" action="browse">
			查询条件：&nbsp;&nbsp;
			用户名:<input type="text" name="username" value="" style="WIDTH:80px">
			年:<select name="year" style="WIDTH: 80px">
			<option value="2008" ${year==2008?"selected":""}>2008年</option>
			<option value="2007" ${year==2007?"selected":""}>2007年</option>
			<option value="2006" ${year==2006?"selected":""}>2006年</option>
		</select>
			月:<select name="month" style="WIDTH: 80px">
	...
	<tr width="100%" height="300px">
		<td width="100%" valign="top">
			...
			<table cellspacing='0' cellpadding='0' width="100%"  border="1">
				<tr class="tabhead">
					<td>用户名</td><td>帐务帐号</td><td>费用（元）</td><td>年</td><td>月</td><td>查看明细</td>
				</tr>
			<c:forEach items="${bills}" var="bill">
			    <tr>
			      <td >${bill.userName}</td>
			      <td >${bill.loginName}</td>
			      <td >${bill.totalFee}</td>
			      <td ><div align="center">${bill.year}</div></td>
			      <td ><div align="center">${bill.month}</div></td>
			      <td align="center" ><a href="details?id=${bill.id}">明细</a></td>
			    </tr>
			</c:forEach>
			</table>
			</div>
	...
	<tr width="100%"  height="20px">
		<!-- 显示分页导航 -->
		<td>第${pag.pageNum}页/共${pag.pages}页&nbsp;&nbsp;<a href='${pageContext.request.contextPath}/browse?page=${pag.firstPage}&username=${bill.userName}&year=${bill.year}&month=${bill.month}'>|第一页</a>|<a href='${pageContext.request.contextPath}/browse?page=${pag.prePage}&username=${bill.userName}&year=${bill.year}&month=${bill.month}'>上一页</a>|<a href='${pageContext.request.contextPath}/browse?page=${pag.nextPage}&username=${bill.userName}&year=${bill.year}&month=${bill.month}'>下一页</a>|<a href='${pageContext.request.contextPath}/browse?page=${pag.lastPage}&username=${bill.userName}&year=${bill.year}&month=${bill.month}'>最后一页</a>|</td>
	</tr>
</table>
